...
namespace TaskManager.DataAccess.CQRS.Queries.Employees
{
    public class GetEmployeeQuery : QueryBase<Employee>
    {
        ...
        {
            var employee = await context.Employees.Where(x => x.CompanyId == CompanyId).Include(x => x.Company).Include(x => x.Board.AssignmentList).SingleOrDefaultAsync(x => x.Id == Id);
            return employee;
        }
    }
}
